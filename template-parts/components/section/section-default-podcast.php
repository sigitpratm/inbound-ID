...
$arguments = [
		"classes"  => [
				"card"      => "grid grid-cols-2 xl:gap-20 justify-between pb-10 px-4 space-y-6 md:space-y-0",
				"section-1" => "col-span-2 md:col-span-1 min-h-[420px] bg-gray-200 rounded-xl",
...
<div class="<?= $args["classes"]["card"] ?>">
	...
	if ( $args["position"] === "left" ):
		...
		<div class="<?= $args["classes"]["section-2"]; ?>">
			<div class="w-full flex flex-col space-y-8 md:space-y-12">
				...
				<?php if ( ! empty( $args["data"]["description"] ) ): ?>
					<div class="<?= $args["data"]["description_classes"] ?>">
						<p class="text-center text-base md:text-base lg:text-lg"><?= $args["data"]["description"]; ?></p>
					</div>
				<?php endif; ?>
		...
		<div class="<?= $args["classes"]["section-2"] ?>">
			<div class="w-full flex flex-col space-y-12">
				...
				<?php if ( ! empty( $args["data"]["btn_url"] ) && ! empty( $args["data"]["btn_text"] ) ): ?>
					<div class="w-full block relative">
						<a href="<?= $args["data"]["btn_url"]; ?>"
						   class="w-full md:w-auto xl:px-10 py-4 bg-[#3D9E26] shadow-lg rounded-full text-white font-bold">
							<?= $args["data"]["btn_text"] ?>
						</a>
					</div>
				<?php endif; ?>
