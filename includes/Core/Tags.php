...
class Tags
{
	...
	public static function posted_on()
	{
		...
		$posted_on = sprintf(
			esc_html_x('Posted on %s', 'post date', __TEXT_DOMAIN__),
			'<a href="'.esc_url(get_permalink()).'" rel="bookmark">'.$time_string.'</a>'
		);
		$byline = sprintf(
			esc_html_x('by %s', 'post author', __TEXT_DOMAIN__),
	...
	public static function entry_footer()
	{
		...
		if ('post' === get_post_type()) {
			...
			$categories_list = get_the_category_list(esc_html__(', ', __TEXT_DOMAIN__));
			if ($categories_list && self::categorized_blog()) {
				printf('<span class="cat-links">'.esc_html__('Posted in %1$s', __TEXT_DOMAIN__).'</span>', $categories_list); // WPCS: XSS OK.
			}
			/* translators: used between list items, there is a space after the comma */
			$tags_list = get_the_tag_list('', esc_html__(', ', __TEXT_DOMAIN__));
			if ($tags_list) {
				printf('<span class="tags-links">'.esc_html__('Tagged %1$s', __TEXT_DOMAIN__).'</span>', $tags_list); // WPCS: XSS OK.
			}
		}
		if (!is_single() && !post_password_required() && (comments_open() || get_comments_number())) {
			echo '<span class="comments-link">';
			/* translators: %s: post title */
			comments_popup_link(sprintf(wp_kses(__('Leave a Comment<span class="screen-reader-text"> on %s</span>', __TEXT_DOMAIN__), array('span' => array('class' => array()))), get_the_title()));
			echo '</span>';
		}
		edit_post_link(
			sprintf(
			/* translators: %s: Name of current post */
				esc_html__('Edit %s', __TEXT_DOMAIN__),
